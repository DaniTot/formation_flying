...
class English:
    ...
    def do_manager(self):
        ...
        if len(self.flight.received_bids) > 0:
            ...
            # TODO communicate highest bid to contrators
            ...
            if self.acceptance_strategy(list(highest_bid.values())[0],
                ...
                print(
                    f"{self.flight.agent_type}, {self.flight.unique_id} selected {highest_bid['bidding_agent'].unique_id}'s bid: {highest_bid['value']}")
            else:
                # Communicate refusal to the contractor agent
                highest_bid["bidding_agent"].english.pending_bids[self.flight]["accepted"] = False
    ...
    def do_contractor(self):
        ...
        # A copy of the bids must be created, in order to avoid issues by removing dict entries while looping.
        refused_bids = []
        accepted_bids = []
        ...
        if self.flight.formation_state is "no_formation" and len(self.managers_calling) >= 1:
            ...
            if selected_manager is not None:
                ...
                print(self.flight.agent_type, self.flight.unique_id, "makes bid to", selected_manager.unique_id,
                      "with value of", selected_bid)
    ...
    # dominant strategy is to bid small amounts more than current highest bid and stop when private value
    # price is reached
    def bidding_strategy(self, fuel_saved):
        self.selected_bid = None
        if highest_bid < fuel_saved:
            self.selected_bid = self.selected_bid + 0.01
    ...
    def call_for_contract(self):
        for neighbor in self.flight.model.space.get_neighbors(pos=self.flight.pos,
                                                              ...
                                                              include_center=True):
            if neighbor.agent_type == "Flight" and neighbor.unique_id != self.flight.unique_id and neighbor.manager == 0 and neighbor.formation_state is "no_formation":
                neighbor.english.managers_calling.append(self.flight)
        return
